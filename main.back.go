...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: ./main config")
		return
	}

	// Check for the "config" argument
	if os.Args[1] == "config" {
		runConfig()
	} else {
		fmt.Println("Unrecognized argument. Usage: ./main config")
	}
...
func runTerraform(terraformDir string, envVars EnvVars, terraformCmd string) error {
	cmd := exec.Command("terraform", "-chdir="+terraformDir, terraformCmd, "-auto-approve", "-var", "repo_name="+envVars.ServiceName, "-var", "gitlab_token="+envVars.GitToken, "-var", "runner_id="+envVars.RunnerID)
	err := cmd.Run()
	if err != nil {
		return err
	}
...
func runConfig() {
	...
	jsonOutput, err := cli.HCLtoJSON(string(hclContent))
	if err != nil {
